...
describe('API tests', () => {
  ...
  it('Get API with plugin: brands', () => {
    cy.api('GET', 'https://qauto.forstudy.space/api/cars/brands').then(
      response => {
  ...
  it('API with plugin-2: models', () => {
    cy.api('GET', 'https://qauto.forstudy.space/api/cars/models').then(
      response => {
        cy.log(JSON.stringify(response.body.data));
        const data = response.body.data;
  ...
  it('Post API with plugin: post a car', () => {
    const auth = {
      email: 'Ksuniasida@gmail.com',
      password: 'zHiqR52i.6iAXV',
      remember: true,
    };
    cy.api('POST', 'https://qauto.forstudy.space/api/auth/signin', auth).then(
      response => {
    ...
    cy.api('POST', 'https://qauto.forstudy.space/api/cars', newCar)
      .as('postResponse')
      .its('body.status')
      .should('eq', 'ok');
